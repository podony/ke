...
class RoadSpeedLimiter:
  def __init__(self):
    ...
    params = Params()
    # boltpilot option defaults (apilot.json "감속제어")
    self.autoNaviSpeedCtrlStart = float(params.get("dp_cam_decel_start", encoding="utf-8") or "18")
    self.autoNaviSpeedCtrlEnd = float(params.get("dp_cam_decel_end", encoding="utf-8") or "6")
    self.autoNaviSpeedBumpDist = float(params.get("dp_cam_decel_bump_dist", encoding="utf-8") or "10")
    self.autoNaviSpeedBumpSpeed = float(params.get("dp_cam_decel_bump_speed", encoding="utf-8") or "30")
    self.autoNaviSpeedSafetyFactor = float(params.get("dp_cam_decel_safety_factor", encoding="utf-8") or "1.05")
  ...
  def get_max_speed(self, cluster_speed, is_metric):
    ...
    try:
      ...
      # phone-sent factor wins, fall back to dp option (boltpilot default 1.05)
      camSpeedFactor = clip(self.roadLimitSpeed.camSpeedFactor or self.autoNaviSpeedSafetyFactor, 1.0, 1.1)
      ...
      if cam_type == 22:  # speed bump
        MIN_LIMIT = 10
        cam_limit_speed = max(cam_limit_speed, int(self.autoNaviSpeedBumpSpeed))
      ...
      if cam_limit_speed_left_dist is not None and cam_limit_speed is not None and cam_limit_speed_left_dist > 0:
        ...
        if cam_type == 22:
          starting_dist = self.autoNaviSpeedBumpDist
          safe_dist = max(v_ego * self.autoNaviSpeedCtrlEnd, 1.)
        else:
