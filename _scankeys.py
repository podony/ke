...
Q = "\x27\""
keys = set()
pat1 = r"(?:params|p)\s*\.\s*(?:get|get_bool|put|put_bool|remove|check_key|get_param_path|all_keys)\(\s*[" + Q + r"]([A-Za-z0-9_]+)"
pat2 = r"put(?:_bool)?_nonblocking\(\s*[" + Q + r"]([A-Za-z0-9_]+)"
...
pat3 = r"^\s*\(\s*[" + Q + r"]([A-Za-z0-9_]+)[" + Q + r"]\s*,"
for m in re.finditer(pat3, mp, re.M):
    keys.add(m.group(1))
